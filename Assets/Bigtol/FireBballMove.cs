...
public class FireBballMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Destroy(gameObject, 0.1f);  //**초뒤 빅파이어볼 비활성화
            Vector3 dir = new Vector3(Camera.main.transform.rotation.x, Camera.main.transform.rotation.y, Camera.main.transform.rotation.z);    //카메라 시점방향?
            ...
            for (int i=0; i<5; i++)
            {
                ...
                ball.transform.LookAt(dir + ball.transform.position) ;
                //ball.transform.LookAt(dir+new Vector3(ball.transform.position.x + tmp, ball.transform.position.y+tmp, ball.transform.position.z)) ;
            }
