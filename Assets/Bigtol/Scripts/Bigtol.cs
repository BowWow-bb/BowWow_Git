...
public class Bigtol : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (HP < 100.0f && Mathf.Abs(transform.position.x - Player.transform.position.x) < 30.0f)
        {
            if (Player.transform.position.x < transform.position.x && transform.position.x - move * Time.deltaTime * move_v > -56.0f)   //플레이어가 빅톨의 왼쪽에 위치, 왼쪽 벽 경계 이동제한
            {
            ...
            else if (Player.transform.position.x > transform.position.x && transform.position.x + move * Time.deltaTime * move_v < 56.0f)  //플레이어가 빅톨의 오른쪽에 위치, 오른쪽 벽 경계 이동제한
            {
                transform.position = new Vector3(transform.position.x + move * Time.deltaTime * move_v, transform.position.y, transform.position.z);
                move_tmp += move * Time.deltaTime * move_v;  //현재 이동거리 업데이트
            }
        ...
        //if (Mathf.Abs(Player.transform.position.x-transform.position.x) < 25.0f)   //플레이어가 일정거리 내 접근한 경우
        {
        ...
        //if (HP % 20 == 0 && HP < 80.0f) //hp에 따른 자동 적용 스킬
        {
            for(int i=0; i<raintol_n; i++)
            {
                ...
                StartCoroutine(RainDelay());    //레인톨 생성 대기
            }
        }
        ...
        //if (HP % 20 == 0 && HP < 70.0f) //hp에 따른 자동 적용 스킬
        {
            for (int i = 0; i < summon_n; i++)
            {
                ...
                StartCoroutine(SummonDelay());//미니톨 생성 대기
            }
        }
    }
    ...
    {
        yield return new WaitForSeconds(0.1f);  //레인톨 생성후 일정시간 대기
    }
    IEnumerator SummonDelay()
    {
        yield return new WaitForSeconds(0.3f);  //미니톨 생성후 일정시간 대기
    }
